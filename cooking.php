...
if(!isset($_SESSION["user_id"])) {
	...
	if($Row_permiss['user_loai']!=6){
		...
		echo '<meta http-equiv="refresh" content="20;URL=cooking.php" />';
		if(isset($_GET["id"])&&is_numeric($_GET["id"])){
			...
			echo '<meta http-equiv="refresh" content="0;URL=cooking.php" />';
		}
?>
<!DOCTYPE html >
<head>
  ...
  <div class="row">
      ...
      <table width="100%" id="tableOrder" border="1" class="table table-striped">
        ...
        <tr class="TH">
      	...
      	<tbody>
        <?php
					$result = mysqli_query($Conn, 'SELECT orderchitiet.*, menu.menu_ten FROM orderchitiet INNER JOIN menu ON orderchitiet.menu_id=menu.menu_id WHERE orderchitiet.orderchitiet_status=3 AND menu.menu_loai=1 ORDER BY orderchitiet_id DESC LIMIT 2');
        ...
        <?php }?>
				<?php
					$result = mysqli_query($Conn, 'SELECT orderchitiet.*, menu.menu_ten FROM orderchitiet INNER JOIN menu ON orderchitiet.menu_id=menu.menu_id WHERE orderchitiet.orderchitiet_status!=3 AND menu.menu_loai=1 ORDER BY orderchitiet_id ASC');
					while($row=mysqli_fetch_array($result, MYSQLI_ASSOC)){
        ?>
          <tr">
            <td align="center"><?php echo $row["order_id"]; ?></td>
